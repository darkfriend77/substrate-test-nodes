...
Once the development environment is set up, build the node dotmog. This command will build the
...
```bash
./target/release/node-dotmog purge-chain --dev
```

Start a dev chain:

```bash
./target/release/node-dotmog --dev
```

Or, start a dev chain with detailed logging:

```bash
RUST_LOG=debug RUST_BACKTRACE=1 ./target/release/node-dotmog -lruntime=debug --dev
```

### Multi-Node Local Testnet

To see the multi-node consensus algorithm in action, run a local testnet with two validator nodes,
Alice and Bob, that have been [configured](/bin/node-dotmog/node/src/chain_spec.rs) as the initial
...
```shell
./target/release/node-dotmog --help
```
...
Review the [FRAME runtime implementation](./runtime/src/lib.rs) included in this dotmog and note
the following:
...
dotmog pallet that is [defined in the `pallets`](./pallets/dotmog/src/lib.rs) directory.
